...
public class ListProcessor {
	...
	public Set<OWLEntity> getMinimalCombination(
			List<List<Set<OWLEntity>>> possibilities, int upperBound) {
		/*
		 * For each (outer list: over axioms) axiom that we need to make local,
		 * we probably have a few different possibilities (inner list: over
		 * different possibilities for a given axiom) as to which set of symbols
		 * we need to remove from the harmlessSignature.
		 * 
		 * We follow a greedy strategy: first, we look at the the possibilities
		 * for those axioms for which there is only one way to proceed - we
		 * assume that these are precisely the first elements in the list,
		 * (better to make all deterministic decisions before we start making
		 * nondeterministic ones) so one must make sure that is the case before
		 * calling this method. Then, we go through the remaining elements in
		 * whatever order they come and keep the "solution" that adds fewer
		 * symbols to the accumulator while still not adding any roles.
		 */

		// plain greedy algorithm
		// System.out.println("getMinimalCombination");
		// System.out.println( possibilities.size() +
		// " axioms need to make local");
		Set<OWLEntity> accumulator = new HashSet<OWLEntity>();
		for (List<Set<OWLEntity>> solutionsForSomeAxiom : possibilities) {// does
																			// this
																			// go
																			// through
																			// the
																			...
																			// natural
																			// order??
			accumulator.addAll(addsFewest(solutionsForSomeAxiom, accumulator,
					upperBound));
		}
		return accumulator;
	}

	public Set<OWLEntity> addsFewest(List<Set<OWLEntity>> list,
			Set<OWLEntity> set, int upperBound) {
		...
		if (list.size() > 0) {
			...
			for (Set<OWLEntity> possibility : list) {
				...
				if (roles < minNoRolesSoFar
						|| (roles == minNoRolesSoFar && adds < chosenAdds)) {
	...
	public Set<OWLEntity> getBestCombination(List<Set<OWLEntity>> list1,
			List<Set<OWLEntity>> list2, int upperBound) {
		...
		for (Set<OWLEntity> s1 : list1) {
			...
			for (Set<OWLEntity> s2 : list2) {
				...
				if ((roles < minNoRolesSoFar)
						|| (roles == minNoRolesSoFar && aux.size() < chosenAdds)) {
					chosenAdds = aux.size();
					minNoRolesSoFar = roles;
					best1 = i1;
					best2 = i2;
	...
	public List<Set<OWLEntity>> getAllCombinations(
			List<List<Set<OWLEntity>>> solsForEachDisjunct, boolean useEvery) {
		...
		if (useEvery) {
			...
			if (total < 100) {
				...
				int walker; // walks along the list commonSolutions filling in
							// the solutions
				int blockSize;
				int index;
				for (int i = 0; i < size; i++) {
					...
					for (int k = i + 1; k < size; k++) {
						blockSize = blockSize
								* filteredSolsForEachDisjunct.get(k).size();
					}
					walker = 0;
					index = 0;
					while (walker < total) {
						commonSolutions.get(walker).addAll(
								solsForOneDisjunct.get(index));
						walker++;
			...
			} else {// if there are too many possible combinations, then we will
					// get only one in a greedy way
				...
				for (List<Set<OWLEntity>> solsForOneDisjunct : filteredSolsForEachDisjunct) {
					greedySolution.addAll(addsFewest(solsForOneDisjunct,
							greedySolution, upperBound));
				}
