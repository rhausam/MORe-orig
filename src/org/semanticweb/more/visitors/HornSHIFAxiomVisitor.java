...
public class HornSHIFAxiomVisitor implements OWLFragmentVisitor {
	...
	public void visit(OWLSubClassOfAxiom axiom) {
		...
		isHornSHIF = classExpVisitor.isSHIF(subClass)
				&& classExpVisitor.isSHIF(superClass);
		if (isHornSHIF) {
			isHornSHIF = containsNoPositiveDisjunction(superClass)
					&& containsNoNegativeDisjunction(subClass)
					&&
					// containsNoPositiveMaxCardGt1(superClass) &&
					// containsNoNegativeMaxCardGt1(subClass) &&
					containsNoPositiveNegation(subClass)
					&& containsNoNegativeNegation(superClass)
					&& containsNoPositiveAllValues(subClass)
					&& containsNoNegativeAllValues(superClass)
					&& containsNoPositiveMinCard(subClass)
					&& containsNoNegativeMinCard(superClass);
			// && containsNoPositiveMaxCard(subClass) &&
			// containsNoNegativeMaxCard(superClass);
		}
	}
	...
	public void visit(OWLEquivalentClassesAxiom axiom) {
		...
		while (isHornSHIF && iterator.hasNext()) {
			...
			isHornSHIF = (classExpVisitor.isSHIF(exp))
					&& containsNoPositiveDisjunction(exp)
					&& containsNoNegativeDisjunction(exp)
					&&
					// containsNoPositiveMaxCardGt1(exp) &&
					// containsNoNegativeMaxCardGt1(exp) &&
					containsNoPositiveNegation(exp)
					&& containsNoNegativeNegation(exp)
					&& containsNoPositiveAllValues(exp)
					&& containsNoNegativeAllValues(exp)
					&& containsNoPositiveMinCard(exp)
					&& containsNoNegativeMinCard(exp);
			// && containsNoPositiveMaxCard(exp) &&
			// containsNoNegativeMaxCard(exp);
		}
	}
	...
	public void visit(OWLDisjointClassesAxiom axiom) {
		...
		// so what we need is for all classExpressions to be suitable for the
		// lhs of a subClassOf axiom
		// isHornSHIF = false;
		SHIFClassExpressionVisitor classExpVisitor = new SHIFClassExpressionVisitor();
		Iterator<OWLClassExpression> iterator = axiom.getClassExpressions()
				.iterator();
	...
	public void visit(OWLObjectPropertyDomainAxiom axiom) {
		...
		// so what we need is for the domain classExpression to be suitable for
		// the rhs of a subClassOf axiom
		// isHornSHIF = false;
		SHIFClassExpressionVisitor classExpVisitor = new SHIFClassExpressionVisitor();
	...
	protected class PosOccurrenceVisitor implements OWLClassExpressionVisitor {
		...
		public void visit(OWLObjectComplementOf ce) {
			if (classExpressionKind.equals("negation"))
				...
				NegOccurrenceVisitor visitor = new NegOccurrenceVisitor(
						classExpressionKind);
		...
		public void visit(OWLObjectExactCardinality ce) {
			...
			// else{
			// if (classExpressionKind.equals("minCard") ||
			// classExpressionKind.equals("maxCardGt1"))
			// contains = (ce.getCardinality() > 1);
			// else{
			// NegOccurrenceVisitor visitor = new
			// NegOccurrenceVisitor(classExpressionKind);
			// contains = containsPosOccurrence(ce.getFiller())
			// || visitor.containsNegOccurrence(ce.getFiller());
			// }
			// }
	...
	private class NegOccurrenceVisitor implements OWLClassExpressionVisitor {
		...
		@Override
		public void visit(OWLObjectComplementOf ce) {
			PosOccurrenceVisitor visitor = new PosOccurrenceVisitor(
					classExpressionKind);
