...
public class ImprovedBottomLocalityChecker implements OWLAxiomVisitor {
	...
	private Set<OWLEntity> globalEntities;// we don't count any solutions
											// containing global entities (don't
											// really need to make any changes
											// to the ListProcessor)

	private BottomChecker negativeVisitor;
	private TopChecker positiveVisitor;

	public ImprovedBottomLocalityChecker() {
	...
	public LocalityInfo isLocalAxiom(OWLAxiom axiom,
			Set<OWLEntity> extSignature, Set<OWLEntity> globalEnts) {
		...
		if (nAlreadyBott > nExps - 2) {
			...
			if (nCanBeBott < nExps - 1) {
				...
				if (nCanBeBott < nExps) {
					...
					canMakeLocal = true;
					solutions = listProc.getAllCombinations(sols, false); // false
																			// because
																			// we
																			// want
																			// solutions
	...
	public void visit(OWLDataPropertyDomainAxiom axiom) {
		...
		// belong to the external signature or the domain is locally top
		isLocal = false;
		canMakeLocal = true; // ultimately we can always make this axiom local
								// by removing the property from the
								// externalSignature
		solutions = new LinkedList<Set<OWLEntity>>();
	...
	private class BottomChecker implements OWLClassExpressionVisitor {
		...
		public void visit(OWLObjectSomeValuesFrom exp) {
			...
			if (!isBottom) {
				...
				if (!externalSignature.contains(prop)) {
					...
					canMakeBottom = true;
					solutions = new LinkedList<Set<OWLEntity>>();
				} else if (!globalEntities.contains(prop)) {
					canMakeBottom = true;
					Set<OWLEntity> aux = new HashSet<OWLEntity>();
					aux.add(prop);
					solutions.add(aux);
		...
		public void visit(OWLObjectMinCardinality exp) {
			...
			// property or the filler are locally negative
			exp.getFiller().accept(this);
			if (!isBottom) {
				OWLObjectProperty prop = exp.getProperty().getNamedProperty();
				if (externalSignature.contains(prop)) {
