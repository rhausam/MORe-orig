...
public class ImprovedLsignatureExtractor extends LsignatureExtractor{
	...
	public Set<OWLEntity> findLsignature(OWLOntology o, Fragment f){
		...
		LogOutput.print("Lsignature extraction took " + tExt + " milliseconds");// and "+ nIterations + " iterations");
		...
		for (OWLAxiom axiom : ontology.getAxioms()) {
			...
			if (fragmentVisitor.isInFragment()){
				...
				if (printAxiomsNotInFragment)
					System.out.println(axiom.toString() + " not in fragment " + fragment.toString());
				
				
				LocalityInfo locInfo = locChecker.isLocalAxiom(axiom, compSignature, globalEntities);
				if (!locInfo.is()) {
					if (locInfo.canMake()) {
						...
						System.out.println("empty lSignature due to axiom " + axiom.toString());
	...
	protected void reduceLsignature(){
		...
		for (OWLAxiom axiom : lSignatureModule) {
			...
			if (!lSignature.containsAll(axiom.getSignature())){
				LocalityInfo locInfo = locChecker.isLocalAxiom(axiom, lSignature, globalEntities);//did I break something by moving this in here?
				if (!locInfo.is()) 
					if (locInfo.canMake()) {
		...
		for (OWLAxiom ax : notGlobalAxioms){
			...
			if (ax instanceof OWLSubClassOfAxiom){
				...
				if (subClass.equals(new OWLDataFactoryImpl().getOWLThing()) ||
					...
					if (!locChecker.isLocalAxiom(ax, ax.getSignature(), new HashSet<OWLEntity>()).canMake()){
		...
		while (!newGlobalEntities.isEmpty()){
			...
			for (OWLAxiom ax : notGlobalAxioms){
				if (containsGlobalEntities(ax,aux)){
					if (!locChecker.isLocalAxiom(ax, new HashSet<OWLEntity>(), globalEntities).canMake()){
						globalAxioms.add(ax);
