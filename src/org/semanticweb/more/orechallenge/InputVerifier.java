...
public class InputVerifier {
	...
	public boolean isValidOperation(String op) {
		...
		List<String> ops = Arrays.asList("sat", "query", "classification",
				"consistency");
		for (String s : ops) {
		...
		if (!isValid)
			System.err
					.println("\tInvalid operation name '"
							+ op
							+ "'. It must be one of: [ classification | sat | query | consistency ]");
	...
	public boolean isParseable(String ontFile) {
		...
		if (!f.exists())
			System.err.println("\tThe given ontology filepath '" + ontFile
			...
			try {
				...
				System.err.println("\tError parsing ontology file '" + ontFile
						+ "' using the OWL API v"
	...
	public boolean isValidConceptName(String cName) {
		...
		if (ont.containsClassInSignature(c.getIRI()))
			...
			System.err.println("\tInvalid concept name. '" + cName
					+ "' is not contained in the ontology signature");
		return isValid;
	}
	...
	public static void main(String[] args) {
		if (!(args.length >= 3)) {
			System.err
					.println("\tEmpty or incomplete argument list. Arguments must be: <Operation> <OntologyFile> <Output> (<Concept> | <QueryFile>)");
			System.exit(0);
		}
		...
		if ((operation.equalsIgnoreCase("classification") || operation
				.equalsIgnoreCase("consistency")) && args.length == 3)
		...
		// Satisfiability
			...
			if (args.length < 4)
				System.err
						.println("\tA concept name URI is required for satisfiability testing");
			else {
				String cName = args[3];
		...
		// Query answering
			...
			if (args.length < 4)
				System.err.println("\tThe query file has not been specified");
			else {
				if (!new File(args[3]).isFile())
					System.err.println("\tThe given query filepath '" + args[3]
							+ "' does not point to a valid file");
				else
					System.out.println("Valid parameters");
			}
